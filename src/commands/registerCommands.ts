...
import { getTodayIsoDate, parseIsoDateOnly } from "../utils/date";
    ...
    context.subscriptions.push(
        ...
        vscode.commands.registerCommand("stockView.openWebsite", () => {
            vscode.env.openExternal(
                vscode.Uri.parse("https://www.eastmoney.com/")
            );
        }),
...
async function handleEditHolding(
    ...
    const currentCostDate = currentConfig?.costDate;
    ...
    if (shares <= 0) {
        await stockDataProvider.updateHolding(
            item.configId,
            0,
            undefined,
            undefined
    ...
    let costDate: string | undefined;

    if (costPrice !== undefined) {
        const defaultDate = currentCostDate || getTodayIsoDate();
        const costDateInput = await vscode.window.showInputBox({
            prompt: `请输入 ${item.label} 的购入时间（可选，影响今日盈亏和总盈亏数据）`,
            placeHolder: "YYYY-MM-DD，留空表示不设置",
            value: defaultDate,
            validateInput: (value) => {
                if (!value.trim()) {
                    return null;
                }
                return parseIsoDateOnly(value.trim())
                    ? null
                    : "日期格式错误，示例：2026-04-23";
            }
        });

        if (costDateInput === undefined) {
            return;
        }
        costDate = costDateInput.trim() ? costDateInput.trim() : undefined;
    }
...
async function handleAddSingleStock(
    ...
    if (shares <= 0) {
        ...
        try {
            ...
            vscode.window.showInformationMessage(
                `已添加股票 ${selectedMarket}.${parsed.code}`
            );
        } catch (error) {
    ...
    const costPrice = costPriceInput.trim()
        ? Number(costPriceInput.trim())
        : undefined;
    let costDate: string | undefined;
    if (costPrice !== undefined) {
        const costDateInput = await vscode.window.showInputBox({
            prompt: "请输入购入时间（可选，影响今日盈亏和总盈亏数据）",
...
async function handleAddBatchStock(
    ...
    if (validItems.length === 0) {
        const reasons = [
            ...
            unknownMarketCodes.length > 0
                ? `无法识别市场 ${unknownMarketCodes.length} 项`
                : ""
        ]
            .filter(Boolean)
            .join("，");
        vscode.window.showErrorMessage(
            `批量添加失败：未找到有效股票代码${reasons ? `（${reasons}）` : ""}`
        );
        return;
    }
...
async function handleAddSector(
    ...
    const code = await vscode.window.showInputBox({
        ...
        validateInput: (value) =>
            /^\d+$/.test(value.trim()) ? null : "代码必须为数字"
    });
    if (code === undefined) {
        return;
    }
...
async function handleAddFuture(
    stockDataProvider: StockTreeDataProvider
): Promise<void> {
    const name = await vscode.window.showInputBox({
        prompt: "请输入期货名称（可选）",
...
async function chooseMarket(
    ...
    const picked = await new Promise<(typeof picks)[number] | undefined>(
        (resolve) => {
...
function resolveMarket(
    explicitMarket: MarketType | undefined,
    code: string
): MarketType | null {
    if (explicitMarket) {
